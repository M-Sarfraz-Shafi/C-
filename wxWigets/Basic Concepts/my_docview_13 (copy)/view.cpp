            ...
            EVT_SIZE(ImageCanvas::OnEventSize)
                EVT_MAXIMIZE(ImageCanvas::OnMaximize)
                    EVT_ICONIZE(ImageCanvas::OnIonized)
                        EVT_MOUSEWHEEL(ImageCanvas::OnZoom)
                            wxEND_EVENT_TABLE()
...
int ImageCanvas::Get_Width()
{
    w = this->GetSize().GetWidth();
    return w;
}

int ImageCanvas::Get_Hight()
{
    h = this->GetSize().GetHeight();
    return h;
}

int ImageCanvas::zoom_delta()
{
    return wheel_delta;
}

void ImageCanvas::set_delta()
{
    wheel_delta = 0;
}

void ImageCanvas::OnZoom(wxMouseEvent &event)
{
    if (event.GetWheelRotation() > 0)
    {
        wheel_delta = wheel_delta + 5;
    }
...
{
    //  draw_clear = 0;
    cout << "Inside Refresh_window  " << endl;
    wxWindow::Refresh();
}
...
bool ImageView::OnCreate(wxDocument *doc, long flags)
{
    ...
    left_window->CreateButton("Fit Image", ID_btn_fit, new wxPoint(60, 200));
...
{
    wxImage image = GetDocument()->GetImage();
    wxImage image_2 = image.Copy();
    int zoom_delta = m_canvas->zoom_delta();
    int r_width = m_canvas->Get_Width();
    int r_hight = m_canvas->Get_Hight();
    int image_width = image.GetWidth();
    int image_hight = image.GetHeight();
    if (fit_button == 1)
    {
        //wxSize *im_size = new wxSize(r_width+zoom_delta, r_hight+zoom_delta);//
        //wxPoint *pt = new wxPoint(0,0);
        //   image.Resize(*im_size, *pt);
        //image.Resize(im_size,&pt,-1,-1,-1);
        //image.Scale(r_width+zoom_delta, r_hight+zoom_delta);
        dc->SetPen(*wxBLACK);
      //  dc->SetBrush(*wxBLUE);
        dc->DrawRectangle(0, 0, r_width, r_hight);
        image.Rescale(r_width, r_hight);
        //image.Resize(wxSize(r_width,r_hight),wxPoint(10,10),-1,0,0);
    ...
    if (fit_button == 0)
    {
        ...
        //dc->SetBrush(*wxBLUE);
        dc->DrawRectangle(0, 0, r_width, r_hight);
        image.Rescale(r_width + zoom_delta, r_hight + zoom_delta);
        // image.Resize(wxSize(r_width,r_hight),wxPoint(10,10),-1,-1,0);
    }

    if (image.IsOk())
    {
        cout << endl
             << "Window w= " << r_width << " : h= " << r_hight << endl;
        cout << "Image w= " << image_width << " : h= " << image_hight << endl;
        cout << "fit value = " << fit_button << endl;
        cout << "Image Draw" << endl;
        dc->DrawBitmap(wxBitmap(image), 1, 1, true);
    }
        ...
        EVT_BUTTON(ID_btn_right, MyWindow::OnRightClicked)
            EVT_BUTTON(ID_btn_up, MyWindow::OnUpClicked)
                EVT_BUTTON(ID_btn_down, MyWindow::OnDownClicked)
                    EVT_BUTTON(ID_btn_fit, MyWindow::OnFitclicked)
                        wxEND_EVENT_TABLE()
...
void MyWindow::OnFitclicked(wxCommandEvent &event)
{
    fit_button = 1;
    cout << "Fit button clicked" << endl;
    if ((wxStaticCast(parentSplitter->GetWindow2(), MyWindow)) != NULL)
    {
        ImageCanvas *ptrMyWin = wxStaticCast(parentSplitter->GetWindow2(), ImageCanvas);
        ptrMyWin->Refresh_Window();
