...
{
    Bind(wxEVT_GESTURE_ZOOM, &ImageCanvas::OnZoom1, this);
    wheel_delta = 0;
}
...
void ImageCanvas::OnZoom1(wxZoomGestureEvent &event)
{
    cout << "Workng" << endl;
}
...
bool ImageView::OnCreate(wxDocument *doc, long flags)
{
    ...
    //   cout<<"client size for right window is : "<<(m_canvas->GetSize())<<endl;
    frame->SetClientSize(m_canvas->GetSize());
    frame->Show();
...
void ImageView::OnDraw(wxDC *dc)
{
    ...
    cout << "Image hight is " << image.GetWidth() << " : " << image.GetHeight() << endl;
    if (image.IsOk())
    {
