...
void ImageCanvas::OnDraw(wxDC &dc)
{
    if (m_view)
    {
        ...
        // {
        //     cout << "------------m = " << m << "------------------" << endl;
        //     cout << "view imagecanvas" << axis1 << endl;
...
void MyWindow::OnDownClicked(wxCommandEvent &event)
{
    ...
    // myImageCanvas->Refresh();
    wxWindow *sibling = GetNextSibling();
    wxSplitterWindow *s_window;
    if (sibling != NULL)
    {
        // void* check =wxStaticCast(ImageCanvas::Refresh_Window(),MyWindow);
        //wxSplitterWindow* wxStaticCast(sibling, wxSplitterWindow);
        //s_window =
        //cout << "Next window get" << endl;
        sibling->Refresh();
    }
...
ImageCanvas *MyWindow::GetRefresh()
{
    // return wxStaticCast(wxScrolledWindow::Refresh_Window(),ImageCanvas); //Not working
}
