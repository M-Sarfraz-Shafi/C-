...
{
    imageBitMap.LoadFile(file, format);
}
...
MyFrame::MyFrame()
    : wxFrame(NULL, wxID_ANY, "Image Handling")
{
...
bool MyApp::OnInit()
{
    ...
    m_imageRGB = new wxImage("image", wxBITMAP_TYPE_ANY, -1);
    //drawPane = new wxImagePanel( frame, wxT("image.jpg"), wxBITMAP_TYPE_JPEG);
    imageBitMap = wxBitmap(*m_imageRGB, -1);
    wxBitmap(*drawPane, -1);
...
//	char str[128] = "" ; // proposed file name
	...
	/*wxString filename = wxFileSelector(_T("Save image as"),_T(""),_T(""),_T("*.bmp"), _T("BMP files (*.bmp)|*.bmp|GIF files (*gif)|*.gif|JPEG files (*jpg)|*.jpg|PNG files (*png)|*.png|TIFF files (*tif)|*.tif|XPM files (*xpm)|*.xpm|All files (*.*)|*.*"), wxSAVE );
	if ( !filename.empty() )
